local eventName = INVENTORY_EXTENSIONS.name.."autoBind"
...
local function OnInventoryChanged(_, bagId, slotIndex, _isNewItem_, _itemSoundCategory_, _inventoryUpdateReason_, _stackCountChange_, _triggeredByCharacterName_, _triggeredByDisplayName_, _isLastUpdateForMessage_, _bonusDropSource_)
...
function IE_AUTO_BIND.Init()
    Init()
end

function IE_AUTO_BIND.Enable(enable)
    local savedVars = INVENTORY_EXTENSIONS.SavedVars
